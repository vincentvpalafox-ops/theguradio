...
- Source file:
  - `staged_remote_changes/wp-content/plugins/gu-scene-archive/includes/class-gu-scene-archive-maintenance.php`
- Bundle file:
...
- Bundle root:
  - `PASS_HANDOFF_0009_2026-04-23/deploy_bundle/`
- Source file last changed in tracked history:
  - `7033223` (`add review output for homepage metadata normalization`)
...
- source and bundle SHA-256 hashes match exactly
- source and bundle byte sizes match exactly
- source and bundle line counts match exactly
- `git diff --no-index --exit-code` reported no content difference beyond line-ending warnings

## Use Rules

- deploy this bundle only if the target plugin is the same `gu-scene-archive` instance represented by the local tracked source
- do not deploy this bundle if the tracked source file changes later without regenerating the bundle
